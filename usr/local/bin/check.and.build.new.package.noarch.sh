      ...
      if [ "$NEWSRPM" == "" ] ; then
        ...
        case $NEWDISTTAG in
          .el7 )
            ...
            echo "  ### $NEWSRPM put in $BUILDDIR/queue.$BUILDTYPE ###" >> $MAILFILE
            ;;
          .el7_0 )
            cp $NEWSRPM $BUILDDIR/queue.$BUILDTYPE.7_0
            echo "  ### $NEWSRPM put in $BUILDDIR/queue.$BUILDTYPE.7_0 ###" >> $MAILFILE
            ;;
          .el7_1 )
            cp $NEWSRPM $BUILDDIR/queue.$BUILDTYPE.7_1
            echo "  ### $NEWSRPM put in $BUILDDIR/queue.$BUILDTYPE.7_1 ###" >> $MAILFILE
            ;;
          .el7_2 )
            cp $NEWSRPM $BUILDDIR/queue.$BUILDTYPE.7_2
            echo "  ### $NEWSRPM put in $BUILDDIR/queue.$BUILDTYPE.7_2 ###" >> $MAILFILE
            ;;
          * )
...
if [ -s $MAILFILE ] ; then
  mail -s "NEW PACKAGES - NOARCH - $TODAY" $EMAILLIST < $MAILFILE
